...
public static class RuntimeEndpoints
{
        ...
        this IEndpointRouteBuilder app)
    {
        ...
        .WithDescription(
            "Returns LocalApi readiness, AI runtime state, model availability, and setup guidance.")
        ...
        app.MapPost("/runtime/ai/setup", async (
            ...
            CancellationToken cancellationToken) =>
        {
            RuntimeSetupStartedResponse started =
                coordinator.StartSetup(cancellationToken);
        ...
        .WithDescription(
            "Starts a background task to download or prepare local AI runtime assets.")
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            await foreach (
                RuntimeSetupProgress progress in coordinator.WatchProgressAsync(
                    setupId,
                    cancellationToken))
            {
                string data = System.Text.Json.JsonSerializer.Serialize(progress);
                string eventName = progress.IsCompleted
                    ? "completed"
                    : progress.IsFailed
                        ? "failed"
                        : "progress";
        ...
        .WithDescription(
            "Streams Server-Sent Events (SSE) representing the progress of the AI runtime setup.")
        ...
        app.MapGet("/runtime/models", async (
            ...
            CancellationToken cancellationToken) =>
            ApiResults.Ok(
                await providers.GetSelectedProvider().ListModelsAsync(cancellationToken),
                context))
        .WithName("ListRuntimeModels")
        .WithTags("Runtime")
        .WithSummary("Lists local AI models.")
        .WithDescription(
            "Returns the local model names discovered by the configured AI model registry.")
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            return ApiResults.Ok(
                new RuntimeProviderListResponse(
                    selected.ProviderId,
                    providerDtos),
                context);
        })
        ...
        .WithDescription(
            "Returns providers known to LocalApi with selected-provider, status, and capability metadata.")
