...
public sealed class AiRuntimeProviderRegistry(
    ...
    IOptions<RuntimeOptions> options) : IAiRuntimeProviderRegistry
{
    private readonly IReadOnlyCollection<IAiRuntimeProvider> providers =
        providers.ToArray();
    ...
    public IAiRuntimeProvider GetSelectedProvider()
    {
        ...
        IAiRuntimeProvider? provider = providers.FirstOrDefault(provider =>
            ...
            || string.Equals(
                ...
                StringComparison.OrdinalIgnoreCase)
            || string.Equals(
                provider.ProviderId.Replace("-", string.Empty, StringComparison.Ordinal),
                configuredProvider,
                StringComparison.OrdinalIgnoreCase));
