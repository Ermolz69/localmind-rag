...
public sealed class LlamaCppEmbeddingGeneratorTests
{
    ...
    {
        float[] expected = Enumerable.Range(0, 1024).Select(x => x / 1024f).ToArray();
        using HttpClient client = new(new JsonResponseHandler(CreateEmbeddingResponse(expected)));
    ...
    {
        string values = string.Join(",", embedding.Select(x => x.ToString(CultureInfo.InvariantCulture)));
        return $$"""
            {
              "object": "list",
              "data": [
    ...
    private sealed class JsonResponseHandler(string json) : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            Assert.Equal("http://127.0.0.1:11435/v1/embeddings", request.RequestUri?.ToString());
