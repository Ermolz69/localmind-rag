...
public sealed class EmbeddingModelStoreTests : IDisposable
{
    ...
    public async Task EnsureDownloadedAsync_Should_Reject_Checksum_Mismatch()
    {
        ...
        EmbeddingModelManifest manifest =
            CreateManifest(Encoding.UTF8.GetBytes("different"));

        EmbeddingModelStore store = CreateStore(content);

        InvalidOperationException exception =
            await Assert.ThrowsAsync<InvalidOperationException>(
                () => store.EnsureDownloadedAsync(manifest));
    ...
    private EmbeddingModelStore CreateStore(byte[] responseBytes)
    {
        ...
        return new EmbeddingModelStore(
            ...
            options,
            new EmbeddingModelCatalog(options),
    ...
    {
        string sha256 =
            Convert.ToHexString(SHA256.HashData(expectedContent))
                .ToLowerInvariant();
    ...
    private sealed class TestPathProvider : IAppPathProvider
    {
        ...
        public string DataDirectory =>
            Path.Combine(root, "runtime", "app", "data");

        public string DatabasePath =>
            Path.Combine(DataDirectory, "knowledge-app.db");
        ...
        public string LogsDirectory =>
            Path.Combine(root, "runtime", "app", "logs");
    }
